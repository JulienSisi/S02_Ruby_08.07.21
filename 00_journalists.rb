...
def sort(element)
    # Pour chaque item listé, compter nb de caractère
    element.each do |adresse|
        element_counted = element.count 
    end

    # Sort by string length
    element.each do |adresse|
        element_counted.sort_by(&:length)
    end
    return sort
end
...
def position(element)
    element
    to_find = ["@epenser"]
    to_find.map {|i| element.find_index(i) } .compact.min # => 1
    return position
end
